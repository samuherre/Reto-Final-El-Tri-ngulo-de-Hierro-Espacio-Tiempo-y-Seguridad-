 ...
 *
 * Interfaz simple por consola. El flujo es:
...
#define MAX_PATH    256
#define MAX_STYLES   64  /* Máximo de StyleEntry por sesión */
...
int main(void) {
    ...
    /* Contexto criptográfico: vive en el stack del proceso (no en disco) */
    ...
    /* Verificar retorno de fgets — elimina warning -Wunused-result */
    ...
    while (running) {
        ...
        /* Verificar retorno de scanf — elimina warning -Wunused-result */
        if (scanf(" %c", &opcion) != 1) {
        ...
        case 'o': {
            ...
            free(text);
            /* Limpiar estilos anteriores al cargar nuevo archivo */
            style_count = 0;
        ...
        case 'e': {
            /* Guardar con pipeline RLE → AES-128-CBC */
            if (!crypto_ctx.initialized) {
                printf("[AVISO] No hay llave configurada. Use [k] para ingresar una.\n");
                break;
            }
            size_t len;
            char *text = gap_get_text(&gb, &len);
        ...
        case 'k': {
            /* Configurar llave criptográfica desde passphrase */
            char passphrase[256];
            ...
            /* Borrar passphrase del stack por seguridad */
    ...
    /* Liberar memoria antes de salir — verificable con valgrind */
    gap_free(&gb);
    /* Borrar la llave criptográfica de RAM de forma segura */
